load('C:\Users\Anya\Desktop\FP_LOCAL\fig1\krok_fig2_corr.mat');
load('C:\Users\Anya\Desktop\FP_LOCAL\fig1\krok_fig2_coher.mat');
load('C:\Users\Anya\Desktop\FP_LOCAL\fig1\krok_fig2_fp2ph.mat');

%% CROSS_CORRELATION
fig = figure; fig.Position(3) = 1375;
clr = {'r','g','b'};
Fs = 50;
nStates = length(coher_achda);
nAn = length(uni);
...
subplot(1,2,1);
    plotme = coher;
    plotme = imgaussfilt(plotme, sm); % filter image with Gaussian smoothing kernel of X std
    h = imagesc(t, f, plotme, [0 1]);
    colorbar; colormap(jet(256));
    title('example coherence ACh/DA'); axis('square')
    xlabel('time (s)'); ylabel('frequency (Hz)');
subplot(1,2,2);
    plotme = rad2deg(-ph); % flip axes so red colors are more prominent
    plotme = imgaussfilt(plotme, sm); % filter image with Gaussian smoothing kernel of X std\
    h = imagesc(t, f, plotme, [-180 180]);
    colorbar; colormap(jet(256));
    title('example phase offset'); axis('square')
    xlabel('time (s)'); ylabel('frequency (Hz)');
movegui(gcf,'center');
