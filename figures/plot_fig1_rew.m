load('C:\Users\Anya\Desktop\FP_LOCAL\krok_fig1_reward_beh.mat')
...
idx = [1:size(a_rew{x,1},2)]';
idx = idx(~isnan(a_rew{x,1}(1,:))); % remove nans
...
clr = {'g','m'};
for y = 1:2
    sp(y) = subplot(1,2,y); hold on
    plot(t_rew, a_rew{x,y}(:,idx), 'Color', [0 0 0 0.1]);
    shadederrbar(t_rew, nanmean(a_rew{x,y},2), SEM(a_rew{x,y},2), clr{y});
    xlabel('time to reward (s)'); xlim([-1 2]); xticks([-1:0.5:2]);
